...
calls and one prompt. On a tool call the hook only applies pack rules.
...
on a prompt. On =PreToolUse= it exits. =ljos hook= on Bash is rules
only. The page [[file:grok-build.org][Grok Build]] is the install.
