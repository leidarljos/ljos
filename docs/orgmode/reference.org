...
| =consensus ID= | consensus, tracker | settle under the pack's rows that apply to the issue, with every persona's anchor |
...
| =policy ARGV...= | policy, pack | the line as it would run, then what the pack knows that bears on it |
| =hook [--limit N]= | pack | the memory hook: hook JSON or argv on stdin; the hits scoring at least 0.6 of the best, preferences first, five at most, each once per runner session |
...
=ljos hook= into it on the prompt event (matcher =*=) by default, or on
the events the table's =hook_events= lists (=PreToolUse= takes the matcher
=Bash=), once each, and drops it from events no longer listed. The hook reads the runner's JSON on stdin
...
when it has none of its own and leaves the unscoped row standing.
