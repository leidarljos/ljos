...
``PostToolUse`` ``additionalContext`` after the first tool. ``ljos hook``
searches on the prompt, holds the text, and emits it once on
``PostToolUse``. ``PreToolUse`` is rules only. There is no ``sync.sh``.
...
A tool call prints nothing unless a prompt has already held pack text.
