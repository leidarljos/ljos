...
`scripts/smoke.sh` runs every loop on scratch stores under one temporary directory: memory, agreement, learning, a sitting and its finish, a second sitting that reopens and a third that resumes, a timeline, release under `LJOS_SEAT`, a rewrite that closes its earlier claim and a consolidation that finds nothing left, an as-of read from before the write, a signed handover received and then refused after one byte of one atom is altered, personas with forecasts and the surprisingly popular reading, a brief, calibration, a rule through the hook and through `policy`, a signed handover and its receipt. It needs the seat binaries on `PATH` and starts a pack writer if none answers. It found its first bug on its first run. `scripts/terra/build.sbatch` and `scripts/terra/smoke.sbatch` are the Slurm scripts that build the seat and consensus crates and run the smoke on the build host, as run.
...
`ljos search TOPIC` prints the pack's hits with score, how many of the pack's scorers named the hit out of how many ran (`2/3`), kind, id, age and text; `-n` for more, `--rerank` for the cross-encoder pass, and `--as-of TIME` (a date or RFC 3339) for the pack as it stood then: what the seat knew when it decided something, memories withdrawn since included and memories learnt since left out. The pack tombstones rather than erases, which is what makes the question answerable. `ljos timeline ISSUE` reads the three stores as one dated list, oldest first: the tracker's logbook (creation, state changes, the claim, every note), the deeds the issue cites with the time each was produced, and the memories the issue's title activates with the time each was written. Each line carries its date, its age, the gap since the line before and the store it came from, so a reader gets time as data rather than stamps to do arithmetic on, and a later line supersedes an earlier one on the same matter. `ljos sitting` prints the last twelve as its `== timeline` section; the `ljos_timeline` tool is the same over MCP. `ljos island CUE` asks the pack which memories a task activates: the top search hits seed a two-hop spread along the pack's entity links, and the cluster comes back strongest first. Not a persona or a view; the island this task touches. `--fire` says the seat went on to use it: the strongest eight fire together and their links gain weight, so the next cue like it walks a heavier path.

`ljos habit NAME VALUE` takes a reading of a habit, a number the seat keeps measuring: a claim of kind `habit` that supersedes the habit's earlier reading and carries it as `was`, with its review due one cadence on (`--every 7d`), so `ljos due` and the hook's nudge say when the next reading is late and `ljos search --as-of` answers what the habit stood at then. `ljos habit` lists every habit as it stands with the change since the last reading. Benchmark scores, latencies and counts are readings, not lessons; the sheet the papers are written from is the pack. `ljos due` lists the atoms whose review clock has run out; `ljos graded ID` marks one recalled (`--lapsed` for the other answer) and the pack reschedules it. This is the spaced-review loop the pack already keeps, reached from the seat.
