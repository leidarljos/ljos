...
- When the work wants readers with views of their own, such as a reviewer
  ...
  whose `--about` domains the issue's title or island names, and every
  persona when none does; a panel that seats everyone on everything is a
  count. Model names on a playbook are optional spawn hints; every member
  still ends with `ljos vote --as` then `ljos consensus`. One playbook
  step per subagent; no resume across phases. Each brief is a file to
  start a subagent from. A panel
