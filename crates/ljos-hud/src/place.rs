...
//! pane asks Sway to float and center the overlay `app_id`.

use std::path::PathBuf;

/// Wayland `app_id` / X11 class the Sway criteria match. Same string as
/// [`crate::install_desktop::OVERLAY_APP_ID`].
pub use crate::install_desktop::OVERLAY_APP_ID;
...
mod tests {
    ...
    fn place_command_names_overlay_app_id() {
        ...
        assert_eq!(OVERLAY_APP_ID, crate::install_desktop::OVERLAY_APP_ID);
    }
    ...
    fn overlay_and_popout_ids_differ() {
        let s = crate::app::overlay_window();
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            assert_eq!(
                pop.platform_specific.application_id,
                crate::install_desktop::APP_ID
            );
    ...
    fn empty_swaysock_is_unavailable() {
        ...
        // Unset may still be unavailable; empty was the case under test.
    }
